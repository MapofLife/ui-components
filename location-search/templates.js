...
angular.module("mol-location-search-autocomplete.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("mol-location-search-autocomplete.html",
    ...
    "	</a></script><input class=form-control autocomplete=on ng-show=regions.available ng-model=regions.selected.name placeholder=\"Search for a region\" typeahead-wait-ms=500 uib-typeahead=\"region for region in regions.available | filter:$viewValue | limitTo:50\" typeahead-on-select=selectRegion($model) typeahead-template-url=ac-region-template.html>");
}]);
